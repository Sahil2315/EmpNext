  ...
  return (
    <div className={`mt-4 w-full h-full rounded-lg bg-indigo-600/20 p-2`}>
        <div className="flex flex-row w-full h-full bg-black/50">
          <div className="flex-1 flex flex-col justify-center items-center border-r-2 border-violet-500/15 my-4">
            <span className="text-2xl font-semibold ">Apply for a Leave</span>
            <input type="text" onChange={(e) => setReason(e.target.value)} defaultValue={reason} className="mt-16 text-center outline-none bg-slate-700/30 text-xl px-2 py-1 w-96 rounded-md" placeholder="Reason"/>
            <div className="flex mt-8 flex-row items-center">
              <span>Category:</span>
              <select onChange={(e) => setCat(e.target.value)} className="ml-4 w-48 cursor-pointer outline-none bg-slate-700/80 text-xl px-2 rounded-md py-1" name="Category">
            ...
            <div className="flex mt-8 text-xl flex-row items-center">
              <span>From:</span>
              <input min={todayString} onChange={(e) => setFrom(e.target.value)} defaultValue={fromDate} className="outline-none cursor-pointer ml-2 px-2 py-1 rounded-md bg-slate-700/80" type="date"/>
              <span className="ml-20">To:</span>
              <input min={fromDate} onChange={(e) => setTo(e.target.value)} defaultValue={toDate} className="outline-none ml-2 px-2 py-1 cursor-pointer rounded-md bg-slate-700/80" type="date"/>
            </div>
            <button onClick={submitNew} className="mt-16 py-1 px-6 text-lg bg-indigo-600/50 rounded-lg hover:bg-indigo-600/90">Submit Application</button>
          </div>
          <div className="w-[500px] flex flex-col">
            <span className="text-2xl font-semibold mx-4 text-center mt-8 border-b border-violet-500/15 pb-4"> Previous Applications</span>
            <div className="mt-4 overflow-y-scroll">
              {
                currList.map((item, index) => {
                  return(
                    <div className="flex py-2 px-4 items-center flex-col mt-2 bg-slate-600/50 mx-4 rounded-md" key={index}>
